...
export function drawTankCannon() {
	...
	ctx.lineWidth = 3;
	drawRoundedRect(ctx, -tank_cannon.size / 2, -tank_cannon.size / 2, tank_cannon.size, tank_cannon.size, 10);


	ctx.fillStyle = selectedAmmo.color2;
	ctx.strokeStyle = selectedAmmo.color1;
	drawRoundedRect(ctx, tank_cannon.size / 2, -arrowHeight / 2, arrowLength, arrowHeight, 3);
	
	// // Drawing the cannon muzzle
	// ctx.fillStyle = selectedAmmo.color2;
	// ctx.strokeStyle = selectedAmmo.color1;
	// ctx.lineWidth = 0;
	// drawRoundedRect(ctx, -tank_cannon.size / 2 + 6.6, -tank_cannon.size / 2 + 7.5, 70, 30, 5);  // Adjust the radius as needed
	//
